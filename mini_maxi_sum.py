...
# -*- coding: utf-8 -*-
"""
Created on Sun Apr 27 21:14:30 2025
...
def miniMaxSum(arr):
    ...
    if all(1<=value<=1e9 for value in arr):
        M, m= max(arr), min(arr)
        S = sum(arr) - m
        s = sum(arr) - M
        return print(str(s),str(S))
    else:
        return print("Constraints failed")
                 

arr = [int(pow(i,3)) for i in range(1,int(1e1))]
